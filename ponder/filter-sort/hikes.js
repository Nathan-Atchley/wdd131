const pokemon = [{ name: "Poochyena", level: 100 }, { name: "Amaura", level: 33 }, { name: "Mimikyu", level: 18 }];
console.log(pokemon.sort(sortPokemon));
...
searchButton.addEventListener("click", e => {
  ...
  // filter hikes array with whatever is in the search input box
  const searchInput = document.getElementById("search").value.toLowerCase();

  const filteredHikes = hikes.filter(hike => {
    return hike.name.toLowerCase().includes(searchInput) || hike.description.toLowerCase().includes(searchInput);
  });
  console.log(filteredHikes);

  //Sort the filtered hikes by difficulty
  const sortedHikes = filteredHikes.sort(sortHikesByDifficulty);
  console.log(sortedHikes);

  // Clear the hike container
  hikeContainer.innerHTML = "";

  // Render the sorted hikes
